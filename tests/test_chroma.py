...
@pytest.mark.asyncio
async def test_get_vector(dummy_store):
    text = "This is a sample text for embedding."
    vector = await dummy_store.get_vector(text)
    assert isinstance(vector, list)
    assert len(vector) > 0
    found = await dummy_store.get_by_id("temp_vector_id_for_embedding_extraction")
    assert found is None  # ensure temp id is not stored permanently
