...
from knwl.models import (GragParams, KnwlBlob, KnwlGragContext, KnwlGraph, KnwlInput, KnwlGragInput, )
...
class GraphRAG(GraphRAGBase):
    ...
    def __init__(self, semantic_graph: Optional[SemanticGraphBase] = None, blob_storage: Optional[BlobStorageBase] = None, chunker: Optional[ChunkingBase] = None, graph_extractor: Optional[GraphExtractionBase] = None, ):
        super().__init__()
    ...
    def validate_services(self) -> None:
        ...
        if self.blob_storage is not None and not isinstance(self.blob_storage, BlobStorageBase):
            raise ValueError("GraphRAG: blob_storage must be an instance of BlobStorageBase.")
    ...
    async def ingest(self, input: str | KnwlInput | KnwlDocument, store_source: bool = False, enable_chunking: bool = True, store_chunks: bool = False, ) -> KnwlGraph | None:
        ...
        if store_source:
            if self.blob_storage is None:
                log.warn("GraphRAG: blob_storage is not configured, cannot store source document.")
            else:
        ...
        if store_chunks:
            if self.blob_storage is None:
                log.warn("GraphRAG: blob_storage is not configured, cannot store chunks.")
            else:
    ...
    async def extract(self, input: str | KnwlInput | KnwlDocument, enable_chunking: bool = True) -> KnwlGragIngestion | None:
        ...
        # ============================================================================================
        if enable_chunking:
            chunks: List[KnwlChunk] = await self.chunker.chunk(document_to_ingest.content, source_key=document_to_ingest.id)
        else:
    ...
    async def get_chunk_by_id(self, chunk_id: str) -> KnwlChunk | None:
        ...
        if self.blob_storage is None:
            return None
        blob: KnwlBlob = await self.blob_storage.get_by_id(chunk_id)
        if blob is None:
            return None
        chunk = KnwlChunk(id=blob.id, data=blob.data, name=blob.name, description=blob.description, origin_id=blob.metadata.get("origin_id") if blob.metadata else None, index=blob.metadata.get("index") if blob.metadata else None, chunk_id=blob.metadata.get("chunk_id") if blob.metadata else None, timestamp=blob.timestamp, )
        return chunk
    ...
    async def get_source_by_id(self, source_id: str) -> KnwlDocument | None:
        ...
        if self.blob_storage is None:
            return None
        blob: KnwlBlob = await self.blob_storage.get_by_id(source_id)
        if blob is None:
            return None
        document = KnwlDocument(id=blob.id, content=blob.data.decode("utf-8"), name=blob.name, description=blob.description, timestamp=blob.timestamp, )
        return document
