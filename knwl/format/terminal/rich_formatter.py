...
class RichFormatter(FormatterBase):
    ...
    def _format_primitive(self, value: Any) -> Text:
        ...
        if isinstance(value, bool):
            return Text(str(value), style=self.theme.SUCCESS if value else self.theme.ERROR)
    ...
    def create_panel(
        ...
        **kwargs
        ...
        return Panel(
            content,
            title=f"[{self.theme.TITLE_STYLE}]{title}[/{self.theme.TITLE_STYLE}]",
            subtitle=f"[{self.theme.MUTED}]{subtitle}[/{self.theme.MUTED}]" if subtitle else None,
    ...
    def create_table(
        ...
        **kwargs
        ...
        """
        table = Table(
            title=f"[{self.theme.SUBTITLE_STYLE}]{title}[/{self.theme.SUBTITLE_STYLE}]" if title else None,
            ...
            **{k: v for k, v in kwargs.items() if k not in ["box", "show_header", "header_style"]}
        )
    ...
    def create_tree(self, label: str, **kwargs) -> Tree:
        ...
        """
        return Tree(
            f"[{self.theme.TITLE_STYLE}]{label}[/{self.theme.TITLE_STYLE}]",
            **kwargs
        )
