...
class NetworkXGraphStorage(GraphStorageBase):
    ...
    def __init__(self, path: str = "memory", format: str = "graphml"):
        ...
        if not self._in_memory and self._path is not None:
            if not self._path.endswith(".graphml"):
                log.warn(f"The configured path '{self._path}' does not end with '.graphml'. Appending the extension.")
                self._path += ".graphml"
            self._path = get_full_path(self._path)
            if os.path.exists(self._path):
                preloaded_graph = NetworkXGraphStorage.load(self._path)
                if preloaded_graph is not None:
                    log.info(f"Loaded graph from {self._path} with {preloaded_graph.number_of_nodes()} nodes, {preloaded_graph.number_of_edges()} edges")
        ...
        for key, value in payload.items():
            ...
            if (not isinstance(value, (str, int, float, bool, list)) and value is not None):
                if isinstance(value, dict):
                    ...
                    payload[key] = json.dumps(cast(BaseModel, value).model_dump(mode="json"))
                else:
    ...
    async def edge_degree(self, edge_or_source_id: str, target_id: str = None) -> int:
        if target_id is None:
            ...
            if edge is None:
                raise KeyError(f"edge_degree: edge with id {edge_or_source_id} not found")
    ...
    async def merge(self, nodes: list[dict], edges: list[dict]) -> None:
        ...
        for edge in edges:
            ...
            if source_id is None or target_id is None:
                raise ValueError("NetworkXStorage: edge must contain 'source_id' and 'target_id'")
