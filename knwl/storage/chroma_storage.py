...
class ChromaStorage(VectorStorageBase):
    ...
    async def get_vector(self, text: str) -> list[float]:
        # Chroma does not expose direct embedding generation, so we do a dummy upsert and get the embedding back
        temp_id = "temp_vector_id_for_embedding_extraction"
        self.collection.upsert(ids=[temp_id], documents=[text])
        result = self.collection.get(ids=[temp_id], include=["embeddings"])
        ...
        if embedding is not None and len(embedding) > 0:
            ...
            self.collection.delete(ids=[temp_id])
            raise ValueError("Failed to retrieve embedding from ChromaDB.")
