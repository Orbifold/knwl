...
class BasicExtraction(ExtractionBase):
    def __init__(self, *args, **kwargs):
        ...
        llm_variant = self.get_param(["extraction", "glean", "llm"], args, kwargs, default="ollama", override=config, )
    ...
    async def extract_records(self, text: str, entities: list[str] = None) -> list[list] | None:
        if not text or text.strip() == "":
            return None
        extraction_prompt = prompts.extraction.full_entity_extraction(text, entity_types=entities)
        found = await self.llm.ask(question=extraction_prompt, key=text, category="extraction")
        if not found or found.answer.strip() == "":
            return None
        parts = split_string_by_multi_markers(found.answer, [prompts.constants.DEFAULT_RECORD_DELIMITER, prompts.constants.DEFAULT_COMPLETION_DELIMITER, ], )
        coll = []
        for part in parts:
